# ——*——coding:utf-8——*——
# author: hhhfccz time:2021/5/19 13:46
import os
import sys
import datetime
import cv2
...
def resize_to_screen(src, maxw=1280, maxh=700, copy=False):
    ...
    if scl > 1.0:
        inv_scl = 1.0 / scl
        return cv2.resize(src, (0, 0), None, inv_scl, inv_scl, cv2.INTER_AREA)
    elif copy:
        img = src.copy()
        return img
    else:
...
def get_mask(name, small, pagemask, masktype):
    if masktype == "text":
        ...
        mask = cv2.adaptiveThreshold(small, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
        ...
        mask = cv2.dilate(mask, np.ones((9, 1), dtype=np.uint8))

        if DEBUG_LEVEL >= 3:
            debug_show(name, 0.2, "dilated", mask)

        mask = cv2.erode(mask, np.ones((1, 3), dtype=np.uint8))
        ...
        mask = cv2.erode(mask, np.ones((3, 1), dtype=np.uint8), iterations=3)

        if DEBUG_LEVEL >= 3:
            debug_show(name, 0.5, "eroded", mask)

        mask = cv2.dilate(mask, np.ones((8, 2), dtype=np.uint8))
...
def main(img):
    ...
    print("loaded img with size: ", img.shape[:2])
...
if __name__ == "__main__":
    img_demo = cv2.imread("demo.jpg", 0)
    main(img=img_demo)
